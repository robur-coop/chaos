...
static int
find_best_regression(double *x, /* independent variable */
                     ...
                     int m, /* number of extra samples in x and y arrays
                               ...
                               extend runs test */
                     int min_samples, /* minimum number of samples to be kept
                                         after changing the starting index to
                                         pass the runs test */
...
CAMLprim value regress_find_median(value x, int n) {
...
CAMLprim value regress_find_best_regression(int runs_samples, int n_samples,
                                            int min_samples, value times_back,
                                            value offsets, value weights,
                                            value est, value vres) {
  ...
  regression_ok = find_best_regression(
      (double *)times_back + runs_samples,
      (double *)offsets + runs_samples, (double *)weights,
      n_samples, runs_samples, min_samples, &est_intercept, &est_slope,
      &est_var, &est_intercept_sd, &est_slope_sd, &best_start, &nruns,
      &degrees_of_freedom);
...
CAMLprim value regress_multiple_regress(value x1, value x2, value y, int n, value res) {
  double b2;
  int result;
