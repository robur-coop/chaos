let run filename destination =
  ...
  let finally () = close_in ic; close_out oc in
  Fun.protect ~finally @@ fun () ->
  let buf = Bytes.create 0x7ff in
  really_input ic buf 0 8;
  let real_length = Bytes.get_int64_be buf 0 in
  if in_channel_length ic < Int64.to_int real_length then
    `Error (false, "truncated memtrace file")
  else
    let rec go = function
      | 0L -> ()
      | remaining ->
          ...
          go (Int64.sub remaining llen)
    in
    go (Int64.sub real_length 8L);
    `Ok ()
...
let image =
  ...
  let parser str =
    if Sys.file_exists str && Sys.is_directory str = false then Ok str
    else error_msgf "%S does not exist" str
  in
  let open Arg in
  required
  & pos 0 (some (conv (parser, Fmt.string))) None
...
let cmd =
  let doc =
    "A tool to transform memory trace from any unikernel to a memtrace file \
     (which can be visualised with $(b,memtrace-viewer))."
  in
  let man =
    [
      `S Manpage.s_description
    ; `P
        "$(tname) is a simple program to transform what is recorded by an \
         unikernel to a memtrace file which can be visualised by \
         $(b,memtrace-viewer)."
    ]
  in
